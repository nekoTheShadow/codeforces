...
public class Main {
    public void exec() {
        ...
        for (int i = 0; i < t; i++) {
            ...
            stdout.println(solve(n, m, a));
        }
    }

    private int solve(int n, int m, int[][] a) {
        Set<List<Integer>> p =  Set.of(List.of(0, 0));
        Set<List<Integer>> q = Set.of(List.of(n - 1, m - 1));
        List<List<Integer>> dp = List.of(List.of(1, 0), List.of(0, 1));
        List<List<Integer>> dq = List.of(List.of(-1, 0), List.of(0, -1));

        int ans = 0;
        for (int time = 0; time < (n+m-1)/2; time++) {
            int[] count = new int[2];
            for (List<Integer> r : p) {
                int x = r.get(0);
                int y = r.get(1);
                count[a[x][y]]++;
            }
            for (List<Integer> r : q) {
                int x = r.get(0);
                int y = r.get(1);
                count[a[x][y]]++;
            }
            ...
            Set<List<Integer>> np = new HashSet<>();
            Set<List<Integer>> nq = new HashSet<>();

            for (List<Integer> r : p) {
                for (List<Integer> d : dp) {
                    int dx = r.get(0) + d.get(0);
                    int dy = r.get(1) + d.get(1);
                    if (0 <= dx && dx < n && 0 <= dy && dy < m) {
                        np.add(List.of(dx, dy));
                    }
                }
            }
            for (List<Integer> r : q) {
                for (List<Integer> d : dq) {
                    int dx = r.get(0) + d.get(0);
                    int dy = r.get(1) + d.get(1);
                    if (0 <= dx && dx < n && 0 <= dy && dy < m) {
                        nq.add(List.of(dx, dy));
                    }
                }
            }
