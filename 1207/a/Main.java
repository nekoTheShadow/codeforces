...
public class Main {
    public void exec() {
        int t = stdin.nextInt();
        while (t-- > 0) {
            long b = stdin.nextLong(); // バンズ
            long p = stdin.nextLong(); // ビーフ
            long f = stdin.nextLong(); // チキン

            long h = stdin.nextLong(); // ハンバーガー
            long c = stdin.nextLong(); // チキンバーガー

            long ans = 0;
            if (h < c) {
                long x = Math.min(b/2, f);
                ans += x * c;
                b -= x*2;
                ans += Math.min(b/2, p) * h;
            } else {
                long x = Math.min(b/2, p);
                ans += x * h;
                b -= x*2;
