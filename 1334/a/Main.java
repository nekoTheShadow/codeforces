...
public class Main {
    public void exec() {
        ...
        for (int i = 0; i < t; i++) {
            ...
            stdout.println(solve(n, p, c));
        }
    }

    public String solve(int n, int[] p, int[] c) {
        for (int i = 0; i < n - 1; i++) {
            if (p[i] > p[i+1]) return "NO";
            if (c[i] > c[i+1]) return "NO";
            if (p[i+1]-p[i] < c[i+1]-c[i]) return "NO";
        }
        for (int i = 0; i < n; i++) {
            if (p[i] < c[i]) return "NO";
        }
        return "YES";
    }
