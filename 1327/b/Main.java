...
public class Main {
    ...
    public void solve(int n, int[][] g) {
        int[] married = new int[n];
        Arrays.fill(married, -1);
        List<Integer> princesses = new ArrayList<>();
        ...
        if (princesses.isEmpty()) {
            stdout.println("OPTIMAL");
        } else {
            int princess = princesses.get(0);
            int prince = IntStream.range(0, n).filter(i -> married[i] == -1).findFirst().getAsInt();
            stdout.println("IMPROVE");
            stdout.println(princess+1, prince+1);
        }
    }
