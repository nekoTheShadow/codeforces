...
public class Main {
    public void exec() {
        Map<Character, Map<Integer, Character>> d = new HashMap<>();
        for (char x = 'a'; x <= 'z'; x++) {
            for (char y = 'a'; y <= 'z'; y++) {
                int dist = Math.abs(x - y);
                d.computeIfAbsent(x, unused -> new HashMap<>()).put(dist, y);
            }
        }
        ...
        for (char c : s.toCharArray()) {
            int max = Collections.max(d.get(c).keySet());
            int m = Math.min(max, k);
            t.append(d.get(c).get(m));
            k -= m;
        }
