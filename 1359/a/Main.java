...
public class Main {
    public void exec() {
        ...
        for (int i = 0; i < t; i++) {
            ...
            int m = stdin.nextInt();
            int k = stdin.nextInt();
            stdout.println(solve(n, m, k));
        }
    }

    public int solve(int n, int m, int k) {
        Deque<Integer> dq = new ArrayDeque<>();
        for (int i = 0; i < n-m; i++) dq.addLast(0);
        for (int i = 0; i < m; i++) dq.addLast(1);

        int h = n / k;
        int[] a = new int[k];
        for (int i = 0; i < h; i++) a[0] += dq.removeLast();
        for (int i = 0; i < h; i++) {
            for (int j = 1; j < k; j++) a[j] += dq.removeFirst();
        }
