...
const INFINITY = math.MaxInt32/2 - 1
...
func exec(stdin *Stdin, stdout *Stdout) {
	for T := stdin.ReadInt(); T > 0; T-- {
		...
		if len(d) == 1 {
			stdout.Println(string(t))
			continue
		}

		stack := NewDeque()
		stack.AppendLast(t[0])

		x := 1
		for x < len(t) {
			ch := stack.PeekLast().(rune)
			if ch == t[x] {
				if ch == '0' {
					stack.AppendLast('1')
				} else {
		...
		ans := []rune{}
		for !stack.IsEmpty() {
			ans = append(ans, stack.PopFirst().(rune))
		}
		stdout.Println(string(ans))
	}
}
