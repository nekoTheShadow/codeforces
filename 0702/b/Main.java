...
public class Main {
    public void exec() {
        int n = stdin.nextInt();
        int m = stdin.nextInt();
        long[] a = stdin.nextLongArray(n); // city
        long[] b = stdin.nextLongArray(m); // tower

        Arrays.sort(b);

        long r = 0;
        for (long v : a) {
            int i = bisectLeft(b, v);
            if (i==0) {
                r = Math.max(r, Math.abs(b[i]-v));
            } else if (i==m) {
                r = Math.max(r, Math.abs(b[i-1]-v));
            } else {
                long d1 = Math.abs(b[i]-v);
                long d2 = Math.abs(b[i-1]-v);
