...
public class Main {
    public void exec() {
        ...
        Deque<Integer> r3 = new ArrayDeque<>();
        Deque<Integer> r4 = new ArrayDeque<>();

        for (int i = 0; i < n; i++) {
            r1.addLast(1 + i*2);
            r2.addLast(1 + n*2 + i*2);
            r3.addLast(1 + n*2 + i*2 + 1);
            r4.addLast(1 + i*2+1);
        }

        List<Integer> a = new ArrayList<>();
        for (int i = 0; i < n; i++) {
            a.add(r2.removeFirst());
            a.add(r1.removeFirst());
            a.add(r3.removeFirst());
            a.add(r4.removeFirst());
        }

        String line = a.stream().filter(v -> v <= m).map(Objects::toString).collect(Collectors.joining(" "));
        stdout.println(line);
    }
