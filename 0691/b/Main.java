...
public class Main {
    ...
    public String f(String s) {
        ...
        for (char c : "AHIMOoTUVvWwXxY".toCharArray()) p.add(c);
        Map<Character, Character> q = Map.of('p', 'q', 'q', 'p', 'b', 'd', 'd', 'b');
        ...
        for (int i = 0; i < n / 2; i++) {
            ...
            if (p.contains(c)) {
                if (c != d) return "NIE";
            } else if (q.containsKey(c)) {
                if (q.get(c) != d) return "NIE";
            } else {
                return "NIE";
            }
        }
        ...
        if (n%2!=0) {
            return p.contains(s.charAt(n/2)) ? "TAK" : "NIE";
        }
