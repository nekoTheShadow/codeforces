...
public class Main {
    public void exec() {
        int t = stdin.nextInt();
        while (t-- > 0) {
            String p = stdin.nextString();
            String h = stdin.nextString();
            stdout.println(solve(p, h));
        }
    }

    public String solve(String p, String h) {
        String s1 = Pattern.compile("").splitAsStream(p).sorted().collect(Collectors.joining());
        for (int i = 0; i <= h.length() - p.length(); i++) {
            String s2 = Pattern.compile("").splitAsStream(h.substring(i, i+p.length())).sorted().collect(Collectors.joining());
            if (s1.equals(s2)) {
                return "YES";
            }
        }
